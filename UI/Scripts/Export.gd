...
export(NodePath) var Csg_combiner_node_path
...
func _ready():
	var _error_a = get_popup().connect("index_pressed", self, "_on_popup_index_pressed")
	...
	var _error_b = file_dialog.connect("file_selected", self, "_on_file_selected")
...
func _save_mesh_to_obj(path : String):
	...
	dir.make_dir_recursive(str(path,"/"))
	var name :Array = path.split("/")
	path = str(path,"/",name[name.size()-1])
	var textures = 0
	exported_textures = []
