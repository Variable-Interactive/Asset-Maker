...
func _ready():
	...
	for butts in texture_container.get_children():
		butts.connect("button_texture", Callable(self, "set_texture"))
	menu_buttons.get_node("File").get_popup().connect("index_pressed", Callable(self, "file_menu"))
	menu_buttons.get_node("Help").get_popup().connect("index_pressed", Callable(self, "Help_menu"))


func start_texture_tab():
	for img in block_textures:
		var new_texture = preload("res://UI/TextureButton.tscn").instantiate()
...
func _on_layer_value_changed(value):
	...
	Main.get_node("Camera3D").global_transform.origin.y += value - Global.layer_no
...
func _on_NewTexture_file_selected(path):
	...
	new_texture.connect("button_texture", Callable(self, "set_texture"))
